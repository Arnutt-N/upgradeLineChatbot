...
async def get_or_create_user_status(db: AsyncSession, user_id: str, display_name: str = None) -> UserStatus:
    """รับหรือสร้างสถานะผู้ใช้ พร้อมอัปเดตชื่อผู้ใช้"""
    ...
    if not user_status:
        ...
        print(f"✅ Created new user: {user_id} with name: {user_status.display_name}")
    else:
        ...
        if display_name and (not user_status.display_name or user_status.display_name.startswith("Customer ")):
            ...
            await db.commit()
            print(f"✅ Updated user name: {user_id} -> {display_name}")
    
    return user_status

async def set_live_chat_status(db: AsyncSession, user_id: str, status: bool, display_name: str = None) -> UserStatus:
    """ตั้งค่าสถานะ live chat ของผู้ใช้"""
    user_status = await get_or_create_user_status(db, user_id, display_name)
    user_status.is_in_live_chat = status
    await db.commit()
    return user_status

async def set_chat_mode(db: AsyncSession, user_id: str, mode: str, display_name: str = None) -> UserStatus:
...
async def get_users_with_messages(db: AsyncSession):
    """โหลดรายการผู้ใช้ที่มีประวัติแชท พร้อมชื่อผู้ใช้"""
    result = await db.execute(
        select(UserStatus.user_id, UserStatus.display_name, UserStatus.is_in_live_chat, UserStatus.chat_mode)
