...
async def get_or_create_user_status(db: AsyncSession, user_id: str) -> UserStatus:
    """รับหรือสร้างสถานะผู้ใช้"""
    result = await db.execute(select(UserStatus).filter(UserStatus.user_id == user_id))
    user_status = result.scalar_one_or_none()
    
    if not user_status:
        user_status = UserStatus(user_id=user_id, is_in_live_chat=False, chat_mode='manual')
        db.add(user_status)
        await db.commit()
...
async def set_live_chat_status(db: AsyncSession, user_id: str, status: bool) -> UserStatus:
...
async def set_chat_mode(db: AsyncSession, user_id: str, mode: str) -> UserStatus:
...
async def get_users_with_messages(db: AsyncSession):
    """โหลดรายการผู้ใช้ที่มีประวัติแชท"""
    result = await db.execute(
        select(UserStatus.user_id, UserStatus.is_in_live_chat, UserStatus.chat_mode)
